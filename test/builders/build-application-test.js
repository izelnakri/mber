...
test.serial('buildApplication() works', async (t) => {
  ...
  t.true(/BUILT: application\.js in \d+ms \[12.70 kB\] Environment: development/g.test(message));
...
test.serial('buildApplication(production) works', async (t) => {
  ...
  t.true(/BUILT: application\.js in \d+ms \[8.62 kB\] Environment: production/g.test(message));
...
test.serial('buildApplication(test) works', async (t) => {
  ...
  t.true(/BUILT: application\.js in \d+ms \[12\.69 kB\] Environment: test/g.test(message));
...
test.serial('buildApplication(demo) works', async (t) => {
  ...
  t.true(/BUILT: application\.js in \d+ms \[8\.61 kB\] Environment: demo/g.test(message));
...
test.serial('buildApplication(custom) works', async (t) => {
  ...
  t.true(applicationJSBuffer.length < APPLICATION_JS_TARGET_BYTE_SIZE);
  t.true(stats.size < APPLICATION_JS_TARGET_BYTE_SIZE);
  t.true(/BUILT: application\.js in \d+ms \[12\.63 kB\] Environment: custom/g.test(message));
