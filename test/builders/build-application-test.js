...
test.serial('buildApplication() works', async (t) => {
  ...
  t.true(/BUILT: application\.js in \d+ms \[13.\d+ kB\] Environment: development/g.test(message));
...
test.serial('buildApplication(production) works', async (t) => {
  ...
  t.true(/BUILT: application\.js in \d+ms \[9.\d+ kB\] Environment: production/g.test(message));
...
test.serial('buildApplication(test) works', async (t) => {
  ...
  t.true(/BUILT: application\.js in \d+ms \[13.\d+ kB\] Environment: test/g.test(message));
...
test.serial('buildApplication(demo) works', async (t) => {
  ...
  t.true(/BUILT: application\.js in \d+ms \[9.\d+ kB\] Environment: demo/g.test(message));
...
test.serial('buildApplication(custom) works', async (t) => {
  ...
  t.true(applicationJSBuffer.length < APPLICATION_JS_TARGET_BYTE_SIZE + 1000);
  t.true(stats.size < APPLICATION_JS_TARGET_BYTE_SIZE + 1000);
  t.true(/BUILT: application\.js in \d+ms \[13.\d+ kB\] Environment: custom/g.test(message));
