...
export default async function(abstractionType, name) {
  ...
  const applicationName = require(`${PROJECT_ROOT}/config/environment`)('development').modulePrefix;
  require(`${__dirname}/../generators/${commandAbstranctionType}.js`).default(
    name, PROJECT_ROOT, applicationName
  );
}
