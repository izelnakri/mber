...
export default async function(defaults={
  ...
  const ENV = (await import(`${projectRoot}/config/environment.js`))(OPTIONS.environment);
  ...
  startBuilder(projectRoot, ENV).then(async (buildConfig) => {
    const startHTTPServer = await import('../runners/start-http-server.js');

    startHTTPServer(buildConfig);

    if (!OPTIONS.watch) {
      const runTestsInShell = await import('../runners/run-tests-in-shell.js');
