    ...
    if (environment === 'test') {
      Object.assign(watcher, {
        ...
        tests: chokidar.watch(`${PROJECT_ROOT}/tests`, { ignoreInitial: true }).on('all', (event, path) => {
          ...
          buildApplication(environment).then(() => webSocket.send(`${event}: ${path}`));
        })
      });
    } else if (environment === 'memserver') {
      Object.assign(watcher, {
        memserver: watchMemServerFolder(environment, WebSocketServer, options)
      });
    }
  });
...
function watchMemServerFolder(environment, webSockerServer, options) {
  return chokidar.watch(`${PROJECT_ROOT}/memserver`, { ignoreInitial: true }).on('all', (event, path) => {
    Console.log(getEventColor(event), path.split(PROJECT_ROOT)[1]);
