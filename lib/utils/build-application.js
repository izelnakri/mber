...
// TODO: throw error if main.js, resolver.js or router.js is missing
import fs from 'fs';
...
import recursiveFileLookup from './recursive-file-lookup';
import countTime from './count-time';
import { formatTimePassed, formatSize } from './asset-reporter';

global.mainContext = global;
const convertHBSToAMD = require('./convert-hbs-to-amd').default;
const writeFileAsync = promisify(fs.writeFile);
const readFileAsync = promisify(fs.readFile);
const statAsync = promisify(fs.stat)

// TODO: application.js is missing that has {{outlet}}
...
export default function(environment='development') {
  return new Promise((resolve) => {
    ...
    recursiveFileLookup(`${PROJECT_ROOT}/src`).then((files) => {
