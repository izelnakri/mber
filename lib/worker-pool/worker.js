...
const countTime = require(`${CWD}/../lib/utils/count-time`).default;
...
const lintJavaScript = require(`${CWD}/../lib/utils/lint-javascript`).default;
...
const { formatTimePassed } = require(`${CWD}/../lib/utils/asset-reporter`).default;
...
parentPort.on('message', async ({ action, fileNames, applicationName, importObject, projectRoot }) => {
  if (action === 'TRANSPILE_JS') {
    ...
    try {
      ...
      Console.log(workerReference, chalk.green('lint passes successfully'));
    } catch (error) {
