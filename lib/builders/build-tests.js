...
export default function(buildConfig={
  ...
  return new Promise(async (resolve, reject) => {
    ...
    Promise.all([
      lookup(`${PROJECT_ROOT}/tests`, ['js', 'ts']),
      lookup(`${PROJECT_ROOT}/src`, ['-test.js', '-test.ts'], { filter: onlyTestFiles })
    ]).then(([testFolderFiles, appFolderTestFiles]) => {
      const testFiles = testFolderFiles.concat(appFolderTestFiles);

      return global.MBER_THREAD_POOL.submit({ files: testFiles, APPLICATION_NAME, projectRoot: PROJECT_ROOT })
      // return Promise.all(testFolderFiles.concat(appFolderTestFiles).map((file) => {
      //   return global.MBER_THREAD_POOL.submit({ files, APPLICATION_NAME })
