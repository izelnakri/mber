...
export default function(entryPoint, applicationName, options={ fastboot: true }) {
  ...
  return new Promise((resolve) => {
    return Promise.all([
      ...
      Promise.all(htmlAssets.map((filePath) => readFileAsync(filePath))).then((fileContents) => {
        ...
        return Promise.all([
          writeAssetMap(hashedAssetMap, options),
...
function writeAssetMap(assetMap, options={ fastboot: true }) {
