...
export default function(buildConfig={
  ...
  return new Promise(async (resolve, reject) => {
    ...
    return readBaseEmber(projectRoot, applicationName, ENV, cliArguments).then((fileContent) => {
...
function readBaseEmber(projectRoot, applicationName, ENV, cliArguments) {
  return new Promise((resolve, reject) => {
    ...
    const baseOperations = [
      ...
      ENV.excludeEmberData ? '' : importAddonFolderToAMD(applicationName, 'ember-data/app', projectRoot),
    ...
    if (cliArguments.fastboot) {
      return Promise.all(baseOperations.concat([
        ...
        emberBaseContentCode, emberDataAppCode, documentationRouterCode, appFastbootAddonCode,
        fastbootAjaxInitializerCode, fastbootErrorHandlerInitializerCode,
        appFastbootAppCode
      ]) => {
        const [AJAX_INITIALIZER_CODE, ERROR_HANDLER_INITIALIZER_CODE] = await Promise.all([
          convertESModuleToAMD(fastbootAjaxInitializerCode.toString(), {
            moduleName: `${applicationName}/initializers/ajax`
          }),
          convertESModuleToAMD(fastbootErrorHandlerInitializerCode.toString(), {
            moduleName: `${applicationName}/initializers/error-handler`
          })
        ]);
    ...
    return Promise.all(baseOperations)
      .then(([emberBaseContentCode, emberDataAppCode, documentationRouterCode]) => {
        resolve(`
          ${emberBaseContentCode}
