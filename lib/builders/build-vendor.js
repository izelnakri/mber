...
import importAddonToAMD from '../transpilers/import-addon-to-amd';
  ...
  return new Promise((resolve) => {
    ...
    if (options.fastboot) {
      return Promise.all([
        ...
        importAddonToAMD(ENV.modulePrefix, 'ember-cli-fastboot/app')
      ]).then(([emberContent, fastbootModules, appFastbootModules]) => {
        // resolve(`${emberContent} ${fastbootModules}`);
        resolve(`
          ${emberContent}
