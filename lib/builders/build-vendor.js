...
function readBaseEmber(ENV, options) {
  return new Promise((resolve) => {
    ...
    const baseOperations = [
      ...
      ENV.excludeEmberData ? '' : importAddonFolderToAMD(ENV.modulePrefix, 'ember-data/app'),
    ];
    ...
    if (options.fastboot) {
      return Promise.all(baseOperations.concat([
        ...
        importAddonFolderToAMD(ENV.modulePrefix, 'ember-cli-fastboot/app') // NOTE: should this be moved? make this loaded on application.js?
      ])).then(([
