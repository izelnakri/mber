    ...
    return readBaseEmber(projectRoot, applicationName, ENV, cliArguments)
      .then((fileContent) => {
        return writeVendorJS(
          ...
          window.EmberENV = ${inspect(
            Object.assign(ENV.EmberENV || {}, { environment: ENV.environment }),
            { depth: null }
          )};
      ...
      .then(() => {
        ...
        if (global.HAS_VENDOR_JS_BUILD_ERROR) {
          say.stop();
          say.speak('vendor.js is now fixed. Well done');
...
function readBaseEmber(projectRoot, applicationName, ENV, cliArguments) {
  return new Promise(async (resolve, reject) => {
    ...
    const baseOperations = [
      ...
      importAddonFolderToAMD(applicationName, '@glimmer/component/app', projectRoot),
      importAddonFolderToAMD(applicationName, '@ember/render-modifiers/app', projectRoot),
      ENV.excludeEmberData
        ? ''
        : importAddonFolderToAMD(applicationName, 'ember-data/app', projectRoot),
      importAddonFolderToAMD(applicationName, '@ember-data/debug/app', projectRoot),
      ...
      return Promise.all(
        ...
        .then(
          async ([
            ...
            glimmerComponentCode,
            emberRenderModifiersCode,
            emberDataAppCode,
            emberDataDebugCode,
