...
function readBaseEmber(projectRoot, applicationName, ENV, cliArguments) {
  return new Promise((resolve, reject) => {
    ...
    const baseOperations = [
      ...
      fs.readFile(`${VENDOR_PATH}/fetch/fetch-shim.js`),
    ...
    if (cliArguments.fastboot) {
      return Promise.all(baseOperations.concat([
        ...
        emberBaseContentCode, fetchShimCode, emberDataAppCode, documentationRouterCode, appFastbootAddonCode,
        fetchFastbootShimCode, fastbootAjaxInitializerCode, appFastbootAppCode
      ]) => {
        const AJAX_INITIALIZER_CODE = await convertESModuleToAMD(fastbootAjaxInitializerCode.toString(), {
    ...
    return Promise.all(baseOperations)
      .then(([emberBaseContentCode, fetchShimCode, emberDataAppCode, documentationRouterCode]) => {
        resolve(`
          ${emberBaseContentCode}
