...
export default async function(ENV={ environment: 'development' }, options={
  hasSocketWatching: true,
  vendorPrepends: null,
  vendorAppends: null,
  fastboot: true,
    ...
    return readBaseEmber(APPLICATION_NAME, ENV, OPTIONS).then((fileContent) => {
      return writeVendorJS(
        ...
        ` ${OPTIONS.vendorPrepends || ''}
          ...
          ${fileContent}
          ${addSocketWatch(OPTIONS.hasSocketWatching, OPTIONS.socketPort)}
...
function addSocketWatch(watching, socketPort) {
  return !watching ? '' : `
    if (typeof FastBoot === 'undefined') {
