...
export default function(environment='development', options={
  watching: false, vendorPrepends: '', vendorAppends: ''
}) {
    ...
    return readFileAsync(`${VENDOR_PATH}/${getEmberVendorBuild(ENV)}`).then((fileContent) => {
      return writeVendorJS(
        ...
        ` ${options.vendorPrepends}
          ...
          ${fileContent}
          ${addGlobals(ENV)}
          ${addSocketWatch(options.watching)}
...
function addGlobals(ENV) { // NOTE: might be unnecessary
  const result = `;window.requirejs = requirejs;window.require = require;window.define = define;`; // NOTE: might be unnecessary
