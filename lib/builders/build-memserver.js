  ...
  return new Promise(async (resolve, reject) => {
    ...
    Promise.all([
      ...
      .then(([baseMemServerCode, memServerInitializerCode, userMemServerFiles]) => {
        return Promise.all([
          baseMemServerCode,
          "window.MemserverModel = require('memserver/model').default;",
      ...
      .then(() => {
        ...
        fs.stat(outputPath).then((stats) => {
          Console.log(
            `${chalk.green('BUILT:')} memserver.js in ${formatTimePassed(timePassed)} [${formatSize(
              stats.size
          ...
          if (!lint) {
            return resolve({
              message: `BUILT: memserver.js in ${timePassed}ms [${formatSize(
                stats.size
              )}] Environment: ${environment}`,
              stats: stats,
            });
...
async function writeMemServerFile(outputPath, content, options = { minify: false }) {
  if (options.minify) {
