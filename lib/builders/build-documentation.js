...
export default function(buildConfig={
  ...
  return new Promise(async (resolve, reject) => {
    ...
    Promise.all([
      ...
      ].concat(files.map((file) => transpileToES5(file, applicationName, reject))));
    }).then((allReadFiles) => {
      return writeDocumentationJS(outputPath, environment, `
      ...
      fs.stat(outputPath).then((stats) => {
        ...
        lookup(`${projectRoot}/documentation`, ['js']).then((fileNames) => {
          global.MBER_THREAD_POOL.submit({ action: 'LINT_JS', fileNames, projectRoot });
        });
      });
...
function importDocumentationCode(applicationName) {
  return new Promise((resolve) => {
    ...
    lookup(`${__dirname}/../addons/mber-documentation/src`, ['hbs', 'ts', 'js']).then((files) => {
      targetFiles = files;
