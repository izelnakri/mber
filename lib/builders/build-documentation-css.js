...
export default function(buildConfig={
  ...
  return new Promise(async (resolve, reject) => {
    ...
    const [vendorScssFiles, scssFiles] = await Promise.all([
      lookup(`${PROJECT_ROOT}/documentation/ui`, 'scss', {
        filter(item) {
    ...
    const styleFiles = [STYLES_ROOT].concat(scssFiles).concat(vendorScssFiles);

    return Promise.all(styleFiles.map((scssFile) => fs.readFile(scssFile)))
      .then((scssContent) => {
