...
export default function(entryPoint, applicationName, options={
  fastboot: true, environment: 'development'
}) {
    ...
    return Promise.all([
      ...
      Promise.all(htmlAssets.map((filePath) => readFileAsync(filePath))).then((fileContents) => {
        ...
        return Promise.all([
          ...
          options.fastboot ? writeFastbootPackageJSON(hashedAssetMap, options.environment) : null,
        ...
        Console.log(`${chalk.green('BUNDLED')}: ${applicationName} in ${chalk.yellow(TIME_SPENT)}`);
...
function writeFastbootPackageJSON(assetMap, environment='development') {
  const ENV = require(`${PROJECT_ROOT}/config/environment.js`)(environment);

  return writeFileAsync(`${PROJECT_ROOT}/dist/package.json`, JSON.stringify({
    dependencies: {},
    fastboot: {
      ...
      hostWhitelist: [],
      manifest: {
        appFiles: [assetMap['assets/application.js']], // NOTE: frontend-fastboot includes styles(weirdly), fastboot ajax & error-handler initializer ?!?
        htmlFile: 'index.html',
