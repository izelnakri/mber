...
export default function(buildConfig={
  ...
  return new Promise(async (resolve) => {
    ...
    return Promise.all([
      ...
      (ENV.environment !== 'production') && CLI_ARGUMENTS.testing ?
        fs.readFile(`${PROJECT_ROOT}/tests/index.html`) : null,
      resetDistFolder(OUTPUT_DIRECTORY)
    ]).then(([indexHTMLBuffer, testHTMLBuffer]) => {
      ...
      const htmlAssets = findInternalAssetsFromHTML(PROJECT_ROOT, indexHTMLContent); // NOTE: assumes assets exist in /tmp directory
      const testsHTMLContent = testHTMLBuffer && transpiledTestHTMLBuffer.toString();
      const testAssets = testHTMLBuffer ? findInternalAssetsFromHTML(PROJECT_ROOT, testsHTMLContent) : []; // NOTE: assumes assets exist in /tmp directory
      const targetAssets = htmlAssets.concat(testAssets)
      ...
      Promise.all(targetAssets.map((filePath) => fs.readFile(filePath))).then((fileContents) => {
        ...
        return Promise.all([
          ...
          testsHTMLContent ? rewriteHTML(testsHTMLContent, hashedAssetMap, `${OUTPUT_DIRECTORY}/tests.html`) : null
