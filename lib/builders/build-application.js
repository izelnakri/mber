    ...
    lookup(`${PROJECT_ROOT}/src`, ['js', 'hbs'], { filter: excludeTestFiles }).then((files) => {
      return Promise.all(files.map((fileName) => {
        return new Promise((resolve) => {
          fs.readFile(fileName).then((fileContents) => {
            if (fileName.endsWith('.js')) {
              resolve(convertESModuletoAMD(fileContents, {
                moduleName: getModuleName(fileName, APPLICATION_NAME)
              }));

              lintJavaScript(fileName, fileContents, buildConfig);
            } else if (fileName.endsWith('.hbs')) {
              return resolve(convertHBSToAMD(fileContents, {
                moduleName: getModuleName(fileName, APPLICATION_NAME)
              }));
            }

            resolve('');
          }).catch((error) => buildError(error, reject, fileName));
        });
      }));
    }).then((contents) => {
