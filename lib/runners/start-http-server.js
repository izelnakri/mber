...
export default async function(
  ...
  if (cliArguments.fastboot) {
    ...
    const sandboxGlobals = IS_MEMSERVER ? (await assignSandboxGlobalsForMemserverFastboot(ENV)) : {};
  ...
  server.listen(port, () => {
    ...
    if (ENV.documentation && ENV.documentation.enabled) {
      Console.log(
        chalk.green(
          `Your styleguide/documentation app runs on http://localhost:${port}${ENV.documentation.path}`
        )
      );
    }
  });
...
async function assignSandboxGlobalsForMemserverFastboot(ENV) {
  ...
  }); // TODO: could this be simply index.html?
