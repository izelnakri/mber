// First: be able to build/simplify with bundlers instead of concat (complex needs to be turned to amd format or resolver rewrite)
// 2 approaches: 1- directly write watcher + compilers in rust
// 2- build threadpool node<->rust communication with neon
// FOR RUST: it needs watcher, message printing, scss/js bundler, linter, websocket communication
...
import buildDocumentation from '../builders/build-documentation';
import buildDocumentationCSS from '../builders/build-documentation-css';
...
export default async function(buildConfig={
  ...
  const CLI_ARGUMENTS = buildConfig.cliArguments || { socketPort: 65511 };
  const ENV = buildConfig.ENV || {
    environment: 'development', modulePrefix: APPLICATION_NAME
  };
  const WebSocketServer = new WebSocket.Server({ port: CLI_ARGUMENTS.socketPort });
  ...
  function buildWatchers(watcher={}) {
    Object.assign(watcher, {
      ...
      eslintRC: watch(`${PROJECT_ROOT}/.eslintrc.js`, () => {
        delete require.cache[require.resolve(`${PROJECT_ROOT}/.eslintrc.js`)];
        buildConfig.jsLinter = new eslint.CLIEngine(require(`${PROJECT_ROOT}/.eslintrc.js`));
      })
    });
      ...
      Object.assign(watcher, {
        documentation: watch(
          ...
          (path) => {
            ...
            return buildDocumentation(buildConfig)
          },
          (path, event) => broadcastMessage(WebSocketServer, `${event}: ${path}`)
        )
      });
  ...
  function reloadApplicationWithMessage(path, event) {
    ...
    broadcastMessage(WebSocketServer, `${event}: ${path}`); // NOTE: could be done in rust
  ...
  function watch(watchPath, buildFunction, callback) {
    return chokidar.watch(watchPath, { ignoreInitial: true }).on('all', (event, path) => {
      if (!global.chokidarBuild) {
        global.chokidarBuild = true; // NOTE: hacky way to ensure one build at a time
