...
export default async function(
  ...
  const webSocketServer = new WebSocketServer({ port: cliArguments.socketPort });
  const watcher = buildWatchers();
  const excludeTestFiles = (item) =>
    !(item.path.endsWith('-test.js') || item.path.endsWith('-test.ts'));
  ...
  function buildWatchers(watcher = {}) {
    Object.assign(watcher, {
      ...
      indexHTML: watch(`${projectRoot}/index.html`, (path, event) =>
        reloadApplicationWithMessage(path, event, distPath)
      ),
      src: watch(
        `${projectRoot}/src`,
        (path) => {
          ...
          } else if (path.endsWith('-test.js') || path.endsWith('-test.ts')) {
          ...
          return buildApplication(buildConfig, false).then(() => {
            lookup(`${projectRoot}/src`, ['js', 'ts'], { filter: excludeTestFiles }).then(
              (fileNames) => {
                return global.MBER_THREAD_POOL.submit({
                  action: 'LINT_JS',
                  fileNames,
        ...
        documentation: watch(
          ...
          (path) => {
            ...
            return buildDocumentation(buildConfig, false).then(() => {
              lookup(`${projectRoot}/documentation`, ['js', 'ts']).then((fileNames) => {
                return global.MBER_THREAD_POOL.submit({
                  action: 'LINT_JS',
                  fileNames,
                  projectRoot
        ...
        memserver: watch(
          ...
          () => {
            return buildMemserver(buildConfig, false).then(() => {
              lookup(`${projectRoot}/memserver`, ['js', 'ts']).then((fileNames) => {
                return global.MBER_THREAD_POOL.submit({
                  action: 'LINT_JS',
                  fileNames,
                  projectRoot
  ...
  function reloadApplicationWithMessage(path, event, distPath) {
    reloadFastboot(cliArguments.fastboot, distPath);
...
function reloadFastboot(hasFastboot, distPath) {
  return hasFastboot ? global.fastboot.reload({ distPath }) : null;
}
