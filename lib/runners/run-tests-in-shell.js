...
const defaults = { port: 4200, debug: false, consoleSocketPort: 65532 };
export default function(options={ port: 4200, debug: false, consoleSocketPort: 65532 }) {
  const OPTIONS = Object.assign({}, defaults, options);

  const WebSocketServer = new WebSocket.Server({ port: OPTIONS.consoleSocketPort });
  ...
  (async () => {
    ...
    await page.evaluate(({ consoleSocketPort }) => {
      const socketConnection = new window.WebSocket(`ws://localhost:${consoleSocketPort}`);

      QUnit.moduleStart((details) => {
        console.log('[', details.name, ']');
        socketConnection.send(JSON.stringify({ event: 'moduleStart', details: details }));
      });

      QUnit.testDone((details) => {
        console.log(getTestStatusCode(details), `${details.module} |`, details.name);
        socketConnection.send(JSON.stringify({ event: 'testDone', details: details }));
      });
    ...
    }, { debugMode: OPTIONS.debug, consoleSocketPort: OPTIONS.consoleSocketPort });
