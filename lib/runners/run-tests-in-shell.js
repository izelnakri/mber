...
export default async function(options={ port: 4200, debug: false, socketPort: 65532 }) {
  ...
  WebSocketServer.on('connection', (webSocket) => {
    if (!OPTIONS.debug) {
      webSocket.on('message', (message) => {
        ...
        if (event === 'testDone') {
          if (details.failed > 0) {
            console.log(chalk.red('✘'), details.name, chalk.gray(`(${turnMStoSecond(details.runtime)}s)`));
            details.assertions.forEach((assertion, index) => {
              if (assertion.result === false) {
                console.log(chalk.red('Assertion #' + (index + 1), ':'), assertion.message || 'This assertion fails');
              }
            });
            ...
            console.log(chalk.green('✔'), details.name, chalk.gray(`(${turnMStoSecond(details.runtime)}s)`));
          }
  ...
  await page.goto(`http://localhost:${OPTIONS.port}/?hidepassed`, { timeout: 0 });
